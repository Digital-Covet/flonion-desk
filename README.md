...
| `BETTER_AUTH_URL`, `BETTER_AUTH_SECRET`, `OAUTH_CLIENT_ID_DESK`, `OAUTH_CLIENT_SECRET_DESK` | For `/login` | Operator OAuth via Digital Covet IAM. `/api/auth/*` fails until all four are set. `BETTER_AUTH_URL` must be the public origin (`https://desk.flonion.com` in production, `http://localhost:3000` locally). The browser client uses relative `/api/auth`, so no `VITE_*` variable is needed or read — never hardcode an absolute fallback. Signing in creates the session cookie that `requireOperatorRead`/`requireOperator` check. |
